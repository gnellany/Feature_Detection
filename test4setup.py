...
def region_of_intrest(image):
    ...
    polygon = np.array([[(100,height),(1550, height),(800,100)]])
...
while(cap.isOpened()):
    ...
    lines = cv2.HoughLinesP(cropped_image, 2, np.pi / 180, 90, np.array([]), minLineLength=40, maxLineGap=5)
    averaged_line = average_slope_intercept(frame, lines)
    line_image = display_lines(frame, averaged_line)
    combo_image = cv2.addWeighted(frame, 0.8, line_image, 1, 1)

    cv2.imshow('Window Box Name', combo_image)
