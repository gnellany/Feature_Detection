...
cv2.createTrackbar("Threshold2","Parameters",175,255,empty)
...
def getContours(img, imgCountours):
    ...
    for cnt in contours:
        ...
        if area > areaMin:
            ...
            cv2.putText(imgCountour, 'Points:' + str(len(approx)), (x+w+20, y+20), cv2.FONT_HERSHEY_COMPLEX,.7,
                        (0,255,0),2)
            cv2.putText(imgCountour, 'Area:' + str(int(area)), (x + w + 20, y + 45), cv2.FONT_HERSHEY_COMPLEX, .7,
                        (0, 255, 0), 2)
...
while True:
    ...
    imgStack = stackImages(0.8,([img,imgGray, imgCountour]))
